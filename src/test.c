...
unsigned char victim_ip_bytes[4];
unsigned char router_ip_bytes[4];
...
int main(void) {
    ...
    while (1) {
        ...
        if(htons(eh->h_proto) == 0x806)
        {
            ...
            // Here add check !victim_ip and !router_ip continue:
			if (memcmp(ah->arp_spa, victim_ip_bytes, 4) != 0 ||
                memcmp(ah->arp_dpa, router_ip_bytes, 4) != 0)
                continue;
